"""
Crop Yield Prediction Streamlit App - Fixed Version
==================================================

Fixed version with proper yield status classification and improved Random Forest predictions.
"""
...
class CropYieldPredictor:
    ...
    def load_models(self):
        ...
        try:
            ...
            # Train models with different parameters for better prediction range
    ...
    def predict_yield(self, input_data, model_name='Random Forest'):
        ...
        try:
            ...
            # Apply prediction range adjustment for Random Forest
            if model_name == 'Random Forest':
                # Scale prediction to better match actual data range
                # Use percentile-based scaling
                if prediction < self.yield_stats['q25']:
                    # Scale lower predictions to use more of the low range
                    prediction = self.yield_stats['min'] + (prediction - self.yield_stats['q25']) * 0.5
                elif prediction > self.yield_stats['q75']:
                    # Scale higher predictions to use more of the high range
                    prediction = self.yield_stats['max'] - (self.yield_stats['q75'] - prediction) * 0.5
                
                # Ensure prediction is within reasonable bounds
                prediction = max(self.yield_stats['min'], min(self.yield_stats['max'], prediction))
...
def main():
    ...
    # Main header
    st.markdown('<h1 class="main-header">Crop Yield Predictor - Fixed</h1>', unsafe_allow_html=True)
    ...
    <div style='text-align: center; color: #666;'>
        <p>Crop Yield Predictor - Fixed | Built with Streamlit & Machine Learning</p>
        <p>Predictions update automatically with proper yield classification</p>
    </div>
    """, unsafe_allow_html=True)
